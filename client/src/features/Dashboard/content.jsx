...
export default function Content() {
    ...
    const { mutate: getQuizzes, isPending: getPending , refetch : refetchQuizzes} = useGetQuizzes({
        onSuccess: (data) => {
            if (data.success) {
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-3 grid-rows-auto gap-10 pt-5">
                {getPending ? "loading" :
                    quizzes.map((quiz) => {
                        return <Quiz key={quiz._id} quiz={quiz} selectedworkspace={selectedworkspace} refetchQuizzes={refetchQuizzes} />
                    })}
            </div>
        </div >
        </>
